# This code plots the phase plot of the inverted pendulum using the joint encoder
# during an inverted pendulum controller.
...
max_motor_torque = 0.035 * 2 # Constrained Motor Torque Limit in Nm is multiplied by 2 to handle high torque from control
K = 1.5 # Gain to compensate for uncertainities

# Logging Control
is_logging = False  # Set to True to enable logging, False to disable
log_folder = "logs"  # Specify the folder to save the CSV
log_filename = "inverted_pendulum_controller.csv"  # Specify the filename for logging
    ...
    try:
        while True:
            try:
                ...
                # Desired Torque for inverting pendulum - 2mglsin(thetha)
                desired_torque = 2 * (mass * gravity * length * np.sin(position) * K) # Desired Torque in Nm
...
async def main():
    try:
        ...
        # Plotting Variables
        if show_plots:
            fig.suptitle('Inverted Pendulum Controller Data', fontsize=12)
